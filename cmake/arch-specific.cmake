...
FUNCTION(CheckArchSpecificTag TagName TagValue)
    IF(${TagName} STREQUAL Tag_CPU_name)
        IF(NOT "${TagValue}" STREQUAL 6)
            SET(TAG_CHECK_FAILED ON PARENT_SCOPE)
            SET(TAG_EXPECTED_VALUE 6 PARENT_SCOPE)
        ENDIF()
    ELSEIF(${TagName} STREQUAL Tag_CPU_arch)
        IF(NOT "${TagValue}" STREQUAL v6)
            SET(TAG_CHECK_FAILED ON PARENT_SCOPE)
            SET(TAG_EXPECTED_VALUE v6 PARENT_SCOPE)
        ENDIF()
...
IF(ARCH_SPECIFIC_NL)
    ...
    FOREACH(l IN LISTS ARCH_SPECIFIC)
        ...
        IF(TAG_CHECK_FAILED)
            MESSAGE(STATUS "[arch-specific] ${CMAKE_MATCH_1}: ${CMAKE_MATCH_2} [expected: ${TAG_EXPECTED_VALUE}]")
            SET(CHECK_FAILED ON)
        ELSE()
...
IF(CHECK_FAILED)
    #MESSAGE(FATAL_ERROR "Invalid architecture-specific data in executable")
ENDIF()
